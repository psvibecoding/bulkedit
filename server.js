...
const SHOPIFY_SCOPES      = process.env.SHOPIFY_SCOPES || 'read_products,write_products,read_inventory,write_inventory,read_locations';
...
app.get('/hp-v2', (req, res) => res.sendFile(path.join(__dirname, 'public', 'hp-v2.html')));
app.get('/hp-v3', (req, res) => res.sendFile(path.join(__dirname, 'public', 'hp-v3.html')));
...
app.get('/api/schedule/recap/:id', async (req, res) => {
  ...
  const rows = changes.map(c => {
    ...
    const lines = [];
    const lbl = (s) => `<span style="color:#9ca3af;font-size:10px;text-transform:uppercase;letter-spacing:.05em;font-weight:600">${s}</span>`;
    const old = (s) => `<span style="text-decoration:line-through;color:#b0b0a8">${s}</span>`;
    const nw  = (s) => `<strong style="color:#1a5c38">${s}</strong>`;
    // Product-level fields — one line each
    Object.entries(c.product || {}).forEach(([f, nv]) => {
      if (f === 'seo') return;
      lines.push(`${lbl(FL2[f]||f)} ${old(fmtField(f,c.before?.[f]))} → ${nw(fmtField(f,nv))}`);
    });
    // Variants — one line per variant, price+compareAt combined on same line
    Object.entries(c.variants || {}).forEach(([vid, v]) => {
      const vb = c.variantsBefore?.[vid];
      const vtag = vb?.title && vb.title !== 'Default Title' ? `<span style="color:#9ca3af"> (${vb.title})</span>` : '';
      const parts = [];
      if (v.price !== undefined) parts.push(`${lbl('Price')} ${old(fmtPrice(vb?.price))} → ${nw(fmtPrice(v.price))}`);
      if (v.compareAtPrice !== undefined) parts.push(`${lbl('Compare')} ${old(fmtPrice(vb?.compareAtPrice))} → ${nw(fmtPrice(v.compareAtPrice))}`);
      if (parts.length) lines.push(`${vtag} ${parts.join(' &nbsp;·&nbsp; ')}`);
    });
    const mfc = (c.metafields||[]).length;
    if (mfc) lines.push(`${lbl('Metafields')} ${mfc} field${mfc!==1?'s':''} updated`);
    const detailHtml = lines.length
      ? `<div style="font-size:12px;margin-top:5px;color:#374151;display:flex;flex-direction:column;gap:3px">${lines.map(l=>`<div style="line-height:1.5">${l}</div>`).join('')}</div>`
      : '';
    return `<tr><td style="padding:12px 0;border-bottom:1px solid #f3f3f1;vertical-align:top"><div style="display:flex;align-items:flex-start">${img}<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600;color:#0e0e0c">${title}</div>${detailHtml}</div></div></td></tr>`;
  }).join('');
